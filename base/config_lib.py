...
Config = collections.namedtuple(
    'Config',
    ['name', 'agents', 'environments', 'experiments', 'n_steps', 'n_seeds', 'rec_freq'])
...
def get_job_config(config_in, job_id):
    ...
    for job_info in iterate_through_config(config_in):
        if job_id == job_info['unique_id']:
            ...
            seed = job_info['seed']
            rec_freq = job_info['rec_freq']
            unique_id = job_info['unique_id']
            exp = job_info['experiment_constructor'](
                agent, env, config_in.n_steps, seed, rec_freq=rec_freq, unique_id=unique_id)
            exp_config = {
